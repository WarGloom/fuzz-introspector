...
def _log_runtime_config() -> None:
    """Log native backend selection and memory configuration at startup."""
    global_backend = backend_loaders.parse_native_backends_env()
    avail_gb = memory_manager.get_available_memory_gb()
    workers = memory_manager.get_recommended_worker_count()
    logger.info(
        "[runtime-config] global_backend=%s available_ram=%.1fGB "
        "recommended_workers=%d",
        global_backend,
        avail_gb,
        workers,
    )
...
def run_analysis_on_dir(
    ...
    if skip_html_report:
        ...
        stage_markers.emit(out_dir,
                           "analysis_plugins",
                           "start",
                           count=len(analyses_to_run))
