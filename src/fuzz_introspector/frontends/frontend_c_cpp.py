...
# pylint: disable=line-too-long,unused-argument,redefined-builtin,too-many-nested-blocks
...
class CppSourceCodeFile(SourceCodeFile):
    ...
    def _process_struct(self, struct: Node, namespace: str) -> None:
        ...
        for child in struct_body_field.children:
            ...
            if (not child_name or not child_name.text or not child_type
                    or not child_type.text):
                continue
...
class FunctionDefinition:
    ...
    def _extract_information(self):
        ...
        while True:
            ...
            if (new_parent.type == "class_specifier"
                ...
                full_name = (
                    new_parent.child_by_field_name("name").text.decode(
        ...
        while True:
            ...
            if tmp_node.child_by_field_name("scope") is not None:
                scope_to_add = (
                    tmp_node.child_by_field_name("scope").text.decode(
                        encoding="utf-8", errors="ignore") + "::")
            ...
            if tmp_node.type == "field_identifier":
                tmp_name = tmp_node.text.decode(encoding="utf-8",
                                                errors="ignore")
                break
            if (tmp_node.child_by_field_name("name") is not None and
                    tmp_node.child_by_field_name("name").type == "identifier"):
    ...
    def _process_field_expr_return_type(self, field_expr: Node,
        ...
        if field.type == "template_method":
            ...
            full_name = (field.text.decode(encoding="utf-8", errors="ignore")
                         if field.text else "")
    ...
    def _process_callsites(self, stmt: Node,
        ...
        # Constructor call statement
            ...
            logger.debug(
                "Extracted declaration: Type `%s` : Name `%s`",
                var_type,
                var_name.text.decode(encoding="utf-8", errors="ignore"),
            )
...
def analyse_source_code(source_content: str,
                        language: str = "c++") -> CppSourceCodeFile:
    """Returns a source abstraction based on a single source string."""
    source_code = CppSourceCodeFile(language,
