...
def capture_source_files_in_tree(directory_tree: str,
    ...
    # Performance Optimization: Use a tuple of extensions for fast endswith checks
    # instead of creating a pathlib.Path object on every iteration.
    ...
    for dirpath, dirnames, filenames in os.walk(directory_tree):
        # Performance Optimization: Modify dirnames in-place to avoid os.walk descending
        # into excluded directories. This saves significant I/O and traversal time.
        dirnames[:] = [
            d for d in dirnames
            if not any(exc in d for exc in EXCLUDE_DIRECTORIES)
        ]
        ...
        # Fast path for empty language extensions
