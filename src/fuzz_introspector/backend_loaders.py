...
BACKEND_GO = "go"
BACKEND_RUST = "rust"
SUPPORTED_BACKENDS = (BACKEND_PYTHON, BACKEND_GO, BACKEND_RUST)
...
def _overlay_repo_candidate(repo_root: str, backend: str) -> str:
    if backend == BACKEND_RUST:
        return os.path.join(
            repo_root,
            "tools",
...
def _overlay_path_candidate(backend: str) -> str:
    if backend == BACKEND_RUST:
        return "native_overlay_backend_rust"
    if backend == BACKEND_GO:
        return "native_overlay_backend_go"
    return ""


def _overlay_discovery_backends(backend: str) -> tuple[str, ...]:
    if backend in (BACKEND_RUST, BACKEND_GO):
        return (backend, )
    if backend == BACKEND_NATIVE:
        return (BACKEND_RUST, BACKEND_GO)
    return ()
...
def _resolve_overlay_native_command_with_fallback(
    ...
    """Resolve overlay backend command from env, then deterministic fallbacks."""
    command = resolve_backend_command(command_env_prefix, backend)
    if command is not None:
        return command, []

    checked_candidates: list[dict[str, Any]] = []
    if command_env_prefix != "FI_OVERLAY":
        return None, checked_candidates

    discovery_backends = _overlay_discovery_backends(backend)
    if not discovery_backends:
        return None, checked_candidates

    repo_root = os.path.abspath(
        os.path.join(os.path.dirname(__file__), os.pardir, os.pardir))
    for candidate_backend in discovery_backends:
        repo_candidate = _overlay_repo_candidate(repo_root, candidate_backend)
        repo_found = (bool(repo_candidate) and os.path.isfile(repo_candidate)
                      and os.access(repo_candidate, os.X_OK))
        ...
        if repo_found:
            return [repo_candidate], checked_candidates

        path_candidate = _overlay_path_candidate(candidate_backend)
        path_resolved = shutil.which(
            path_candidate) if path_candidate else None
        checked_candidates.append({
            "backend": candidate_backend,
            "source": "path",
            "candidate": path_candidate,
            "resolved": path_resolved,
            "found": bool(path_resolved),
        })
        if path_resolved:
            return [path_resolved], checked_candidates

    return None, checked_candidates


def _build_overlay_command_env_hint(command_env_prefix: str,
                                    backend: str) -> str:
    if backend in (BACKEND_GO, BACKEND_RUST):
        return (f"Set {command_env_prefix}_{backend.upper()}_BIN, "
                f"{command_env_prefix}_{BACKEND_NATIVE.upper()}_BIN or "
...
def resolve_overlay_backend_command_with_details(
    ...
    command, checked_candidates = _resolve_overlay_native_command_with_fallback(
        command_env_prefix, selected_backend)
    details = {
        "backend":
        selected_backend,
        "execution_backend":
        ...
        checked_candidates,
        "env_hint":
        _build_overlay_command_env_hint(command_env_prefix, selected_backend),
    }
...
def parse_correlator_backend_env(
    ...
    raw = os.environ.get(env_name, "").strip().lower()
    if not raw:
        return parse_native_backends_env()
...
def parse_native_backends_env(env_name: str = "FI_NATIVE_BACKENDS", ) -> str:
    ...
    ``'python'`` (the default when the variable is unset or invalid).

    When set, this value acts as the default for all per-component backend
    selections.  Per-component env vars (e.g. ``FI_DEBUG_CORRELATE_NATIVE``)
    still take explicit precedence via :func:`resolve_component_backend`.
    """
    raw = os.environ.get(env_name, "").strip().lower()
    if not raw:
        return BACKEND_PYTHON
    if raw in (BACKEND_RUST, BACKEND_GO, BACKEND_PYTHON):
        return raw
    logger.warning(
        "Invalid %s=%r; supported values are rust/go/python. Defaulting to python",
        env_name,
        raw,
    )
    return BACKEND_PYTHON
...
def resolve_component_backend(
    component_env_var: str,
    default_backend: str = BACKEND_PYTHON,
) -> str:
    """Resolve the effective backend for a specific component.

    Priority (highest to lowest):

    1. ``component_env_var`` — the per-component override (e.g.
       ``FI_DEBUG_CORRELATE_NATIVE``) if explicitly set and valid.
    2. ``FI_NATIVE_BACKENDS`` — the global unified flag.
    3. ``default_backend`` argument (default: ``'python'``).

    Only the values ``'rust'``, ``'go'``, and ``'python'`` are accepted.
    Invalid values are silently ignored (with a warning) and resolution
    continues to the next priority level.

    Example::

        backend = resolve_component_backend("FI_DEBUG_CORRELATE_NATIVE")
        # Returns 'rust' if FI_DEBUG_CORRELATE_NATIVE=rust, or
        # if FI_NATIVE_BACKENDS=rust (and FI_DEBUG_CORRELATE_NATIVE unset),
        # or 'python' otherwise.
    """
    _valid = frozenset((BACKEND_PYTHON, BACKEND_RUST, BACKEND_GO))

    raw_component = os.environ.get(component_env_var, "").strip().lower()
    if raw_component:
        if raw_component in _valid:
            return raw_component
        logger.warning(
            "Invalid %s=%r; ignoring component override",
            component_env_var,
            raw_component,
        )

    global_backend = parse_native_backends_env()
    if global_backend != BACKEND_PYTHON:
        return global_backend

    if default_backend in _valid:
        return default_backend
    logger.warning(
        "Invalid default_backend=%r for %s; using python",
        default_backend,
        component_env_var,
    )
    return BACKEND_PYTHON
            ...
            while True:
                ...
                if _loop_iteration > _max_loop_iterations:
                    logger.error(
                        ...
                        "forcing termination (set FI_DEBUG_CORRELATOR_TIMEOUT_SEC or adjust "
