...
def add_func_to_reached_and_clone(
    ...
    # Incrementally update unreached complexity. Instead of doing a full O(N*M)
    # recalculation from scratch, we identify newly reached functions and subtract
    # their cyclomatic complexity from any function that reaches them.
    logger.info("Updating hitcount-related data")
    if newly_reached:
        newly_reached_map = {
            f.function_name: f.cyclomatic_complexity
            for f in newly_reached
        }

        # A function's unreached complexity includes its own complexity if it is unreached.
        # If it was newly reached, we first subtract its own complexity from itself.
        for f in newly_reached:
            f.new_unreached_complexity -= f.cyclomatic_complexity

        # Then, subtract the complexity of all newly reached functions from their
        # callers' unreached metrics.
        for f_profile in all_functions.values():
            covered_cc = sum(
                newly_reached_map.get(name, 0)
                for name in f_profile.functions_reached)
            if covered_cc > 0:
                f_profile.new_unreached_complexity -= covered_cc
