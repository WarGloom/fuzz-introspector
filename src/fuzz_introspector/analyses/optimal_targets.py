...
def add_func_to_reached_and_clone(
    ...
    if f.hitcount == 0 and f.cyclomatic_complexity == func_to_add.cyclomatic_complexity:
        f.hitcount = 1
        newly_reached.append(f)
    elif f.cyclomatic_complexity == func_to_add.cyclomatic_complexity:
        f.hitcount = 1
    ...
    logger.info("Updating hitcount-related data incrementally")
    for f_profile in all_functions.values():
        for f_reached in newly_reached:
            # f_profile.new_unreached_complexity was initialized as:
            #   sum(reached.cc for reached in functions_reached if reached.hitcount == 0)
            #   + (f_profile.cc if f_profile.hitcount == 0 else 0)
            # If f_reached just transitioned to hitcount == 1, we must subtract its CC.
            if f_reached.function_name in f_profile.functions_reached:
                f_profile.new_unreached_complexity -= f_reached.cyclomatic_complexity
