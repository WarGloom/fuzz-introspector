...
class FuzzCalltreeAnalysis(analysis.AnalysisInterface):
    ...
    def analysis_func(
        ...
        if analysis.NativePluginProxy.is_enabled():
            try:
                ...
                ).run_analysis(proj_profile, profiles, ["calltree_analysis"])
                rows = native_result["calltree_analysis"]["tables"][
                    "calltree_nodes"]
                if rows:
                    row = rows[0]
    ...
    def html_create_dedicated_calltree_file(
        ...
        complete_html_string_parts = []
        blocker_infos = {}
        # HTML start
        complete_html_string_parts.append(
            html_helpers.html_get_header(
                title=f"Fuzz introspector: {profile.identifier}"))
        complete_html_string_parts.append(
            "<div class='content-wrapper calltree-page'>")
        complete_html_string_parts.append(
        ...
        if fuzz_blocker_table is not None:
            complete_html_string_parts.append('<div class="report-box">')
            complete_html_string_parts.append("<h1>Fuzz blockers</h1>")
            complete_html_string_parts.append(fuzz_blocker_table)
            complete_html_string_parts.append("</div>")

        complete_html_string_parts.append(calltree_html_string)

        # HTML end
        # close html header and content-section calltree-content-section
        complete_html_string_parts.append("</div></div>")

        if len(blocker_infos) > 0:
            complete_html_string_parts.append("<script>")
            complete_html_string_parts.append(
        ...
        complete_html_string_parts.append("</body></html>")

        complete_html_string = "".join(complete_html_string_parts)
    ...
    def create_branch_blocker_table(
        ...
        # Performance Optimization: Replaced O(N^2) string concatenation with O(N) list joins.
        html_parts = []
        html_parts.append("<p class='no-top-margin'>The followings are "
        ...
        for entry in branch_blockers:
            ...
            if file_link is not None:
                ...
                cs_link = ('<span class="text-link" '
                           f"onclick=\" scrollToNodeInCT('{node_id}')\">"
                           f"call site: {node_id}</span>")
            collapsible_id = (entry.source_file +
                              entry.blocked_side_line_numder + random_suffix)
