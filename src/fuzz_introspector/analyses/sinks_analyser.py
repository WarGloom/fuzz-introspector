...
class SinkCoverageAnalyser(analysis.AnalysisInterface):
    ...
    def _retrieve_data_list(
        ...
        # Optimization: Use a set for O(1) membership lookups to prevent O(N^2) bottleneck
        seen_function_names: set[str] = set()
        fuzzer_name_list: list[str] = []

        for key, function in proj_profile.all_functions.items():
            if key not in seen_function_names:
                function_list.append(function)
                seen_function_names.add(function.function_name)
