...
def _resolve_correlator_backend() -> str:
    """Select the correlator backend with alias support.

    Checks ``FI_DEBUG_CORRELATE_NATIVE`` first; if it holds a recognised
    backend name (``rust`` or ``go``) that value is used directly and a log
    message records the alias resolution.  Otherwise the canonical
    ``FI_DEBUG_CORRELATOR_BACKEND`` variable is consulted via
    :func:`backend_loaders.parse_correlator_backend_env`.
    """
    ...
    if raw in (backend_loaders.BACKEND_RUST, backend_loaders.BACKEND_GO):
        ...
        return raw
    if raw and raw not in (backend_loaders.BACKEND_PYTHON, "0", "false", "no",
                           ""):
        logger.warning(
            "[correlator] FI_DEBUG_CORRELATE_NATIVE=%r is not a recognised "
            "backend; ignoring (use 'rust' or 'go')",
            raw,
        )
