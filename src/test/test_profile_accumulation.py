...
class _ProfileStub:
    ...
    def accummulate_profile(
        self, base_folder, _return_dict, _uniq_id, _semaphore, skip_propagation=False
    ):
...
def test_parse_profile_worker_count_rust_backend_default_from_global_env(monkeypatch):
    monkeypatch.delenv(analysis.FI_PROFILE_WORKERS_ENV, raising=False)
    monkeypatch.delenv(analysis.FI_REACHABILITY_BACKEND_ENV, raising=False)
...
def test_parse_profile_worker_count_rust_backend_env_override_respected(monkeypatch):
...
def test_accummulate_profiles_parallel_preserves_input_order(monkeypatch):
    ...
    def fake_accummulate_single_profile(
        profile_index, profile_payload, base_folder, skip_propagation=False
    ):
    ...
    monkeypatch.setattr(
        analysis.concurrent.futures, "ProcessPoolExecutor", _ExecutorStub
    )
    monkeypatch.setattr(
        analysis.concurrent.futures, "as_completed", lambda futures: list(futures)[::-1]
    )
    monkeypatch.setattr(
        analysis, "_accummulate_single_profile", fake_accummulate_single_profile
    )
    monkeypatch.setattr(analysis, "_resolve_profile_worker_count", lambda: (3, False))
    monkeypatch.setattr(
        fuzzer_profile, "propagate_reachability_native_batch", lambda profiles: False
    )

    profiles = [_ProfileStub("first"), _ProfileStub("second"), _ProfileStub("third")]
    result_profiles = analysis._accummulate_profiles(
        profiles, "/tmp/base", parallelise=True
    )
...
def test_accummulate_profiles_parallel_raises_contextual_error(monkeypatch):
    ...
    def fake_accummulate_single_profile(
        profile_index, profile_payload, _base_folder, skip_propagation=False
    ):
    ...
    monkeypatch.setattr(
        analysis.concurrent.futures, "as_completed", lambda futures: list(futures)
    )
    monkeypatch.setattr(
        analysis, "_accummulate_single_profile", fake_accummulate_single_profile
    )
    monkeypatch.setattr(analysis, "_resolve_profile_worker_count", lambda: (2, False))
    monkeypatch.setattr(
        fuzzer_profile, "propagate_reachability_native_batch", lambda profiles: False
    )
...
def test_profile_accumulation_parallel_serial_parity(monkeypatch):
    ...
    serial_result = analysis._accummulate_profiles(
        serial_profiles, "/tmp/base", parallelise=False
    )
    parallel_result = analysis._accummulate_profiles(
        parallel_profiles, "/tmp/base", parallelise=True
    )
...
def test_get_all_functions_with_source_returns_cached_mapping():
    ...
    merged_profile.all_functions = {
        "keep": SimpleNamespace(has_source_file=True, function_linenumber="10"),
        "drop-nosrc": SimpleNamespace(has_source_file=False, function_linenumber="20"),
        "drop-noline": SimpleNamespace(has_source_file=True, function_linenumber="-1"),
    }
...
def test_parse_bool_env_invalid_uses_default(monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING"):
        parsed_value = analysis._parse_bool_env(analysis.FI_DEBUG_STAGE_RSS_ENV, False)
...
def test_parse_stage_warn_rss_mb_invalid_uses_default(monkeypatch, caplog):
    ...
    assert any(
        "FI_DEBUG_STAGE_WARN_RSS_MB" in record.message for record in caplog.records
    )
...
def test_log_debug_load_stage_emits_rss_warning(caplog, monkeypatch):
    ...
    assert any("FI_DEBUG_CORRELATE_PARALLEL" in message for message in warning_messages)
...
def test_batch_returns_false_on_non_success_status(monkeypatch):
    ...
    response = json.dumps({"status": "error", "reason": "something went wrong"})
    monkeypatch.setattr(
        fuzzer_profile.subprocess,
        "run",
        lambda *a, **kw: SimpleNamespace(returncode=0, stdout=response, stderr=""),
    )
    profiles = [_FuzzerProfileStub("p1", {"f": []})]
    assert fuzzer_profile.propagate_reachability_native_batch(profiles) is False
...
def test_batch_applies_results_to_profiles(monkeypatch):
    ...
    p = _FuzzerProfileStub("fuzzer_a", {"foo": ["bar"], "bar": ["baz"], "baz": []})
    result = fuzzer_profile.propagate_reachability_native_batch([p])

    assert result is True
    assert set(p.all_class_functions["foo"].functions_reached) == {"bar", "baz"}
...
def test_deserialize_function_profile_interns_functions_reached():
    ...
    restored = fuzzer_profile.FuzzerProfile._deserialize_function_profile(payload)
        ...
        [
            {
                "profile_id": "fuzz1",
                "results": [
                    {"name": "a", "functions_reached": ["b"], "function_depth": 1}
                ],
            },
            {
                "profile_id": "fuzz2",
                "results": [
                    {"name": "x", "functions_reached": ["y"], "function_depth": 1}
                ],
            },
        ]
    )
...
def test_batch_ignores_unknown_profile_ids(monkeypatch):
    ...
    response = _make_batch_success_response(
        [
            {
                "profile_id": "ghost",  # not in the input list
                "results": [
                    {"name": "f", "functions_reached": ["g"], "function_depth": 1}
                ],
            }
        ]
    )
...
def test_batch_sends_correct_payload(monkeypatch):
    ...
    fuzz_a_entry = next(
        pr for pr in payload["profiles"] if pr["profile_id"] == "fuzz_a"
    )
...
def test_accummulate_profiles_calls_batch_and_passes_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        fuzzer_profile, "propagate_reachability_native_batch", fake_batch
    )
    ...
    class _TrackingStub(_ProfileStub):
        def accummulate_profile(
            self, base_folder, _rd, _uid, _sem, skip_propagation=False
        ):
...
def test_accummulate_profiles_skips_batch_in_parallel_multi_worker(monkeypatch):
...
def test_accummulate_profiles_keeps_batch_for_parallel_single_worker(monkeypatch):
    ...
    monkeypatch.setattr(analysis, "_resolve_profile_worker_count", lambda: (1, False))
