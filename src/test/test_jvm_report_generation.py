...
def test_full_jvm_report_generation(tmpdir, testcase):
    ...
    try:
        ...
        exit_code, _ = commands.run_analysis_on_dir(
            tmpdir, coverage_link, analyses_to_run, "", False, project_name, "jvm"
        )
        ...
        check_fuzz_report(
            tmpdir, class_name, files_reached, files_covered, reached, unreached
        )
    finally:
...
def check_calltree_view(report_dir, files, class_name, fuzzing_method):
    ...
    for file in [f for f in files if f.startswith("calltree_view_")]:
        ...
        depth = int(
            retrieve_tag_content(first_line.find_class("node-depth-wrapper")[0])
        )
        assert depth == 0
        ...
        expected_depth = (
            len(expected_last_line) - len(expected_last_line.lstrip(" "))
        ) / 2
        actual_depth = int(
            retrieve_tag_content(last_line.find_class("node-depth-wrapper")[0])
        )
        ...
        for element in reversed(elements):
            element_depth = int(
                retrieve_tag_content(element.find_class("node-depth-wrapper")[0])
            )
            if element_depth == actual_depth - 1:
                for link_element in element.find_class("coverage-line-filename")[
                    0
                ].getchildren():
                    if retrieve_tag_content(link_element) == "[function]":
                        ...
                        break
                if parent_class != "#":
                    parent_lines = retrieve_tag_content(
                        element.find_class("language-clike")[0]
                    )
        ...
        actual_link = ""
        for link_element in last_line.find_class("coverage-line-filename")[
            0
        ].getchildren():
        ...
        if not parent_class or parent_class == "#":
            ...
            assert (
                actual_link
                == f"{coverage_link}/{parent_class}.java.html#L{expected_lineno}"
            )


def check_function_list(
    report_dir, expected_reached_method, expected_unreached_method, file
):
...
def check_fuzz_report(
    report_dir, class_name, files_reached, files_covered, func_reached, func_unreached
):
