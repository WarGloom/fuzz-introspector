...
def _prepare_fake_repo(tmp_path: Path) -> Path:
    ...
    tools_dir.mkdir()
    for tool_dir in _GO_TOOL_DIRS + _RUST_TOOL_DIRS:
        tool_root = tools_dir / tool_dir
        tool_root.mkdir()
        (tool_root / "README.md").write_text(f"# {tool_dir}\n",
                                             encoding="utf-8")
        if tool_dir.endswith("_go"):
            (tool_root / tool_dir).write_text("compiled", encoding="utf-8")
        else:
...
def test_build_with_fixes_copies_and_wires_go_native_backends(
        tmp_path: Path) -> None:
    ...
    for tool_dir in (
        ...
        assert not (copied_tools / tool_dir / tool_dir).exists()

    dockerfile = (repo_root / "oss_fuzz_integration" / "oss-fuzz" / "infra" /
                  "base-images" / "base-builder" /
                  "Dockerfile").read_text(encoding="utf-8")
    assert "native_reachability_go" in dockerfile
    assert "native_filter_functions_go" in dockerfile
