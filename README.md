...
- Overlay backend minimal recommended set:
  - `FI_OVERLAY_BACKEND` (default: `python`): `python`, `native`, `rust`.
...
- Overlay backend advanced aliases (optional):
  ...
  - `FI_OVERLAY_BACKEND=rust` is a compatibility alias for the native backend
    family.
  - Rollout gate decision (refreshed 2026-03-01): keep
    `FI_OVERLAY_BACKEND=python` as default/authoritative. The native Rust
    backend remains parity-validated on the local introtest fixture, but the
...
- `FI_DEBUG_YAML_LOADER` (default: `rust`): backend for debug YAML loader.
  Supported shipped selectors: `python`, `rust`.
- `FI_PROFILE_YAML_LOADER` (default: `rust`): backend for profile YAML
  parsing. Supported shipped selectors: `python`, `rust`.
...
- Backend binary configuration:
  ...
  - Resolve `FI_DEBUG_CORRELATOR_<BACKEND>_BIN` first, then
    `FI_DEBUG_CORRELATOR_BIN`; `FI_DEBUG_CORRELATOR_RUST_BIN` remains a
    compatibility alias.
  where `<BACKEND>` is uppercase (`RUST` for the shipped native backend).
- Backend policy:
  ...
  - Go native implementations are intentionally not shipped in this branch.
