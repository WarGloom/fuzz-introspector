...
RUN apt-get update && \
    ...
    cd /fuzz-introspector/tools/native_yaml_loader_go && /usr/local/go/bin/go build -o /opt/fuzz-introspector/bin/native_yaml_loader_go . && \
...
ENV FI_DEBUG_YAML_LOADER=rust \
    ...
    FI_DEBUG_CORRELATOR_GO_BIN=/opt/fuzz-introspector/bin/native_debug_correlator_go \
    ...
    FI_OVERLAY_GO_BIN=/opt/fuzz-introspector/bin/native_overlay_backend_go \
