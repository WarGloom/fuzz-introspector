...
class MockPath:
    def __init__(self, filename):
        self.suffix = os.path.splitext(filename)[1]

os.makedirs("test_tree_3/src/main", exist_ok=True)
os.makedirs("test_tree_3/build/bin", exist_ok=True)
os.makedirs("test_tree_3/node_modules/lib", exist_ok=True)
for i in range(10):
    with open(f"test_tree_3/src/main/file{i}.cpp", "w") as f: f.write("")
    with open(f"test_tree_3/src/main/file{i}.h", "w") as f: f.write("")
    with open(f"test_tree_3/build/bin/file{i}.cpp", "w") as f: f.write("")
    with open(f"test_tree_3/node_modules/lib/file{i}.cpp", "w") as f: f.write("")

exts = [".c", ".cpp", ".cc", ".c++", ".cxx", ".h", ".hpp", ".hh", ".hxx"]

start = time.time()
for _ in range(100):
    res1 = capture_source_files_in_tree_orig("test_tree_3", exts)
end = time.time()
print(f"Original: {end - start:.4f}s (found {len(res1)})")

start = time.time()
for _ in range(100):
    res2 = capture_source_files_in_tree_opt("test_tree_3", exts)
end = time.time()
