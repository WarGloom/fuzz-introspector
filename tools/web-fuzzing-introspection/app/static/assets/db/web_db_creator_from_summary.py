...
MUST_INCLUDES = set()
MUST_INCLUDE_WITH_LANG: List[Dict[str, Any]] = []
    ...
    try:
        line_total_summary["percent"] = round(
            100.0 * (float(line_total_summary["covered"]) /
                     float(line_total_summary["count"])),
            2,
        )
    except Exception:
    ...
    if debug_report:
        ...
        for elem in all_files_in_project:
            ...
            if source_file.endswith(".h"):
                ...
                if ("/usr/local/" in normalized_file
                        or "/usr/include/" in normalized_file):
                    continue
...
def extract_project_data(project_name, date_str, should_include_details,
    ...
    # - introspector
    # - both
    if not ((code_coverage_summary is not None and cov_fuzz_stats is not None)
            or introspector_report is not None):
        return
    ...
    if cov_fuzz_stats is not None:
        ...
        for ff in all_fuzzers:
            ...
            try:
                fuzzer_coverage_error_log = (
                    oss_fuzz.get_fuzzer_target_coverage_error_log(
...
def update_db_files(
    ...
    if should_include_details:
        ...
        with zipfile.ZipFile("db-archive.zip", "w") as zip_object:
            zip_object.write(
                os.path.join(output_directory, DB_JSON_DB_TIMESTAMP),
                DB_JSON_DB_TIMESTAMP,
...
def analyse_set_of_dates(
    dates,
    projects_to_analyse,
    output_directory,
    force_creation,
    must_include_not_in_ossfuzz,
):
    ...
    for date in dates:
        ...
        if (not force_creation and is_end is False
                and is_date_in_db(date, output_directory)):
...
def create_db(
    ...
    analyse_set_of_dates(
        date_range,
        projects_to_analyse,
        output_directory,
        force_creation,
        must_includes_not_in_ossfuzz,
    )
