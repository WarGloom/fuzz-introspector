...
def capture_source_files_in_tree_opt2(directory_tree: str, language_extensions: list[str]) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(directory_tree):
        ...
        # Use an explicit loop instead of any() for performance
        excluded = False
        for exclude in EXCLUDE_DIRECTORIES:
            if exclude in dirpath:
                excluded = True
...
os.makedirs("test_tree_4/src/main", exist_ok=True)
os.makedirs("test_tree_4/build/bin", exist_ok=True)
os.makedirs("test_tree_4/node_modules/lib", exist_ok=True)
os.makedirs("test_tree_4/aflplusplus/sub/dir", exist_ok=True)
for i in range(500):
    with open(f"test_tree_4/src/main/file{i}.cpp", "w") as f: f.write("")
    with open(f"test_tree_4/src/main/file{i}.h", "w") as f: f.write("")
    with open(f"test_tree_4/build/bin/file{i}.cpp", "w") as f: f.write("")
    with open(f"test_tree_4/node_modules/lib/file{i}.cpp", "w") as f: f.write("")
    with open(f"test_tree_4/aflplusplus/sub/dir/file{i}.cpp", "w") as f: f.write("")

exts = [".c", ".cpp", ".cc", ".c++", ".cxx", ".h", ".hpp", ".hh", ".hxx"]

start = time.time()
for _ in range(100):
    res1 = capture_source_files_in_tree_orig("test_tree_4", exts)
end = time.time()
print(f"Original: {end - start:.4f}s (found {len(res1)})")

start = time.time()
for _ in range(100):
    res2 = capture_source_files_in_tree_opt("test_tree_4", exts)
end = time.time()
print(f"Optimized: {end - start:.4f}s (found {len(res2)})")

start = time.time()
for _ in range(100):
    res3 = capture_source_files_in_tree_opt2("test_tree_4", exts)
end = time.time()
print(f"Optimized2: {end - start:.4f}s (found {len(res3)})")
